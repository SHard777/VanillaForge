...
def log_sentiment(ticker: str, score: int, theme: str) -> str:
    ...
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO sentiment_history (ticker, score, theme) VALUES (?, ?, ?)",
            (ticker.upper(), score, theme),
        )
...
def get_sentiment_trend(ticker: str) -> List[Dict[str, Any]]:
    ...
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT timestamp, score, theme FROM sentiment_history WHERE ticker = ? ORDER BY timestamp DESC LIMIT 30",
            (ticker.upper(),),
        )
...
@mcp.tool()
def log_paper_trade(
    ticker: str, strategy: str, bsm_price: float, volatility: float, agent_notes: str
) -> str:
    """Logs a hypothetical paper trade into the trade journal."""
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO trade_journal (ticker, strategy, bsm_price, volatility, agent_notes) VALUES (?, ?, ?, ?, ?)",
...
def view_trade_journal() -> List[Dict[str, Any]]:
    ...
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT id, timestamp, ticker, strategy, bsm_price, volatility, agent_notes FROM trade_journal ORDER BY timestamp DESC"
        )
        rows = cursor.fetchall()
        return [dict(row) for row in rows]
